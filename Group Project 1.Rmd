...
par(mfrow=c(2,2))
plot(density(classroom2$zeta0), main ="Normality Check for Zeta")
plot(density(classroom2$eta0), main = "Normality Check for Eta")
#looking strange
qqnorm(classroom2$zeta0);qqline(classroom2$zeta0)
qqnorm(classroom2$eta0);qqline(classroom2$eta0)
#looking strange
```
...
par(mfrow=c(2,2))
plot(density(classroom2$zeta0c), main ="Normality Check for Zeta")
plot(density(classroom2$eta0c), main = "Normality Check for Eta")
# eta looks pretty normal
#zeta not so much
qqnorm(classroom2$zeta0c, main = "Q-Q Plot for Zeta");qqline(classroom2$zeta0c)
qqnorm(classroom2$eta0c, main = "Q-Q Plot for Eta");qqline(classroom2$eta0c)
#eta looking good, but with a few possible outliers
#zeta is horrendous
```
...
```{r complex outliers}
#look for schools more than +/- 2 sds (tails seem to be where outliers are)
meanzeta0c <- mean(classroom2$zeta0c)
sdzeta0c <- sd(classroom2$zeta0c)
which(abs(classroom2$zeta0c)>abs((meanzeta0c+2*sdzeta0c)))
#27 schools may be outliers
```
...
In model 1 , the varaince for $\sigma_{\epsilon}^2 = 1288$ and in model 2 $\sigma_{\epsilong_0}^2 = 602.2$. We note that including child-level variation leads to a decrease in the variance of both the random effects. 
