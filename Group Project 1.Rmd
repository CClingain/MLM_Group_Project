...
\textbf{Quesiton:} For UCM, write down: $V_C$, $V_S$, $V_E$ for the three variance components (simply the estimates). Think of them as possibly varying with a covariate, though.

\textbf{Answer:} 

\textbf{Quesiton:} For the most complicated (all fixed effects) random INTERCEPTS ONLY model, what are: $V_C$, $V_S$, $V_E$?

\textbf{Answer:} 

\textbf{Quesiton:} By what fraction did these each decrease with the new predictors in the model?
