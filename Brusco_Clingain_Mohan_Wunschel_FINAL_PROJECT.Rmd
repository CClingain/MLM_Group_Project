...
There seems to be a trend here that the zeta0/zeta1 ratio is = 1.868107, so let's filter it out.

```{r}
outliers <- classroom2 %>% filter(round(zeta0c/zeta1c,6)==1.868107) %>%
  select(zeta0c,zeta1c,schoolid,minority)
```
Now let's make sure the IDs from the plot are showing up here.
```{r}
unique(outliers$schoolid)
```
They are! Now what's going on with minority ($\zeta_1$)?
```{r}
table(outliers$minority)
```
All the students are minorities!
...
```{r part4 residuals,fig.width=6,fig.height=4}
